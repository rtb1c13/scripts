...
def main():
  ...
  ftp=args.ftp
  table=rangeimport("../Abysis/Light_FR_CDR_table_1.txt")
  heavyobs=[]
  for line in table:
    pdbid=line[0]
    ...
    if structure1[0].has_id('L'):
      heavy1=structure1[0]['L']
    elif structure1[0].has_id('A'):
      heavy1=structure1[0]['A']
    else:
      print "PDB %s has no chain L or chain A. Skipping editing of light chain." % pdbid
      break
    try: heavy2=structure2[0]['L']
    except KeyError: print "Cannot find chain L in Kabat numbered structure of %s.\n This PDB should be edited separately!" % pdbid
...
# Find consensus start point of heavys
  ...
  for i in heavyobs:
    ...
    io.save("Created/%s_L_edited.pdb" % i[0], CASelect())
  ...
  for delres in range(cdrlist[4],cdrlist[5]+1,1):
    if heavy1.has_id(delres):
      for j in [ i for i in heavy1 if i.id[1] == delres ]:
        heavy1.__delitem__(j.get_id())
  if heavy1.has_id(10):
    print "Deleting residue 10 from PDB %s" % heavy1.get_parent().get_parent().get_id()
    for j in [ i for i in heavy1 if i.id[1] == 10 ]:
      heavy1.__delitem__(j.get_id())
# Delete Hetatm records
#  for j in [ i for i in heavy1 if i.id[0] is not ' ' ]:
#    heavy1.__delitem__(j.get_id())
